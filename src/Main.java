...
public class Main {
	...
	public static String kingdomNumber(Kingdom k) {
		return "k" + k.getNumber() + "  " + k.getDate();
	}

	public static void createNewdir(Kingdom k) {
		String path = PATH_OF_PIC + "\\" + kingdomNumber(k);
	...
	public static Player convertToObj(String kingdom, int rank) {
		...
		if (lines.length > 11)
			p.setT5(lines[11]);
		/**
		 * File f; f = new File(PATH_OF_PIC + "\\" + kingdom + "\\" + rank + "\\" +
		 * ID_PIC + ".png"); if (f.exists() && !f.isDirectory())
		 * p.setId(textFromPic(kingdom, rank, ID_PIC)); f = new File(PATH_OF_PIC + "\\"
		 * + kingdom + "\\" + rank + "\\" + POWER_PIC + ".png"); if (f.exists() &&
		 * !f.isDirectory()) p.setPower(textFromPic(kingdom, rank, POWER_PIC)); f = new
		 * File(PATH_OF_PIC + "\\" + kingdom + "\\" + rank + "\\" + DEAD_PIC + ".png");
		 * if (f.exists() && !f.isDirectory()) p.setDeaths(textFromPic(kingdom, rank,
		 * DEAD_PIC)); f = new File(PATH_OF_PIC + "\\" + kingdom + "\\" + rank + "\\" +
		 * BARBS_PIC + ".png"); if (f.exists() && !f.isDirectory())
		 * p.setBarbs(textFromPic(kingdom, rank, BARBS_PIC)); f = new File(PATH_OF_PIC +
		 * "\\" + kingdom + "\\" + rank + "\\" + HELP_PIC + ".png"); if (f.exists() &&
		 * !f.isDirectory()) p.setHelps(textFromPic(kingdom, rank, HELP_PIC)); f = new
		 * File(PATH_OF_PIC + "\\" + kingdom + "\\" + rank + "\\" + RSS_A_PIC + ".png");
		 * if (f.exists() && !f.isDirectory()) p.setRss_a(textFromPic(kingdom, rank,
		 * RSS_A_PIC)); f = new File(PATH_OF_PIC + "\\" + kingdom + "\\" + rank + "\\" +
		 * RSS_G_PIC + ".png"); if (f.exists() && !f.isDirectory())
		 * p.setRss_g(textFromPic(kingdom, rank, RSS_G_PIC)); f = new File(PATH_OF_PIC +
		 * "\\" + kingdom + "\\" + rank + "\\" + T1_PIC + ".png"); if (f.exists() &&
		 * !f.isDirectory()) p.setT1(textFromPic(kingdom, rank, T1_PIC)); f = new
		 * File(PATH_OF_PIC + "\\" + kingdom + "\\" + rank + "\\" + T2_PIC + ".png"); if
		 * (f.exists() && !f.isDirectory()) p.setT2(textFromPic(kingdom, rank, T2_PIC));
		 * f = new File(PATH_OF_PIC + "\\" + kingdom + "\\" + rank + "\\" + T3_PIC +
		 * ".png"); if (f.exists() && !f.isDirectory()) p.setT3(textFromPic(kingdom,
		 * rank, T3_PIC)); f = new File(PATH_OF_PIC + "\\" + kingdom + "\\" + rank +
		 * "\\" + T4_PIC + ".png"); if (f.exists() && !f.isDirectory())
		 * p.setT4(textFromPic(kingdom, rank, T4_PIC)); f = new File(PATH_OF_PIC + "\\"
		 * + kingdom + "\\" + rank + "\\" + T5_PIC + ".png"); if (f.exists() &&
		 * !f.isDirectory()) p.setT5(textFromPic(kingdom, rank, T5_PIC));
		 **/
		return p;
	}
